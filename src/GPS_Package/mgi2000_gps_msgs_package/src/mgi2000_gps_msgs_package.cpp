...
double mgi2000_yaw = 0.0;
double mgi2000_yaw_offset = 0.0;
...
void mgi2000_gps1_Callback(const sensor_msgs::NavSatFix::ConstPtr& gps_msg)
{
...
void mgi2000_heading_Callback(const geometry_msgs::QuaternionStamped::ConstPtr& msg)
{
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "mgi2000_gps_package");
  ros::NodeHandle n;
  ...
  ros::param::get("~mgi2000_yaw_offset", mgi2000_yaw_offset);
  ros::param::get("odom_frame_id", odom_frame_id);
  ros::param::get("odom_child_frame_id", odom_child_frame_id);  
  ros::param::get("imu_topic", imu_topic);     
  
   
  ros::Subscriber sub_mgi2000_fix       = n.subscribe("/fix",5,&mgi2000_gps1_Callback);  // front gps 
  ros::Subscriber sub_mgi2000_heading   = n.subscribe("/heading",5,&mgi2000_heading_Callback);  // rear  gps
  
  ros::Subscriber subIMU = n.subscribe(imu_topic, 50, &imuCallback);  // imu
  ...
  {	
	...
	{
		ROS_FATAL_STREAM("GPS is not FIXED !!");
		
	}
	printf("mgi2000_yaw_offset : %6.3lf\n",mgi2000_yaw_offset);
	loop_rate.sleep();
    ros::spinOnce();
